...
function App() {
  ...
  useEffect(() => {
    const services = servicesItem.slug  ? servicesItem.slug : urlAddres.pathname.match('(\/s-).*?\/')[0].slice(3).replace(/\//g, '')
    const brends = brendsItem.slug ? brendsItem.slug : urlAddres.pathname.match('(\/b-).*?\/')[0].slice(3).replace(/\//g, '')
    const style = styleItem.slug ? styleItem.slug : urlAddres.pathname.match('(\/st-).*?(\/||\s)$')[0].slice(4).replace(/\//g, '')
    navigate(`/s-${services}/b-${brends}/st-${style}/`)
  },[servicesItem, brendsItem, styleItem, navigate]);

  useEffect(() => {
    const paramS = urlAddres.pathname.match('(\/s-).*?\/')[0].slice(3).replace(/\//g, '')
    const paramB = urlAddres.pathname.match('(\/b-).*?\/')[0].slice(3).replace(/\//g, '')
    const paramSt = urlAddres.pathname.match('(\/st-).*?(\/||\s)$')[0].slice(4).replace(/\//g, '')
    axios
